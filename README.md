...
- Works with `Sell`, `Buy`, `Mine`, `Collect Drops`, `Deposit Inventory`, `Update Trade Offers`, `Salvage at Position` and `Deliver Salvage` vanilla orders.
...
- Only `Collect Drops` and `Update Trade Offers` orders are supported to work standalone. 
- De-facto you can have any combination of orders, but as stated before - only two of them can be used in "single mode".
